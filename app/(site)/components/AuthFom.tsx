...
type Variant = 'LOGIN'|'REGISTER';//typescript allows the Generic types
const AuthForm =()=> {

  const[variant,setVariant] = useState<Variant>('LOGIN');
  const[isLoading,setIsLoading] = useState(false);//used for disabling inputs and buttons after we have submitted our form
  ...
  const onSubmit: SubmitHandler<FieldValues> = (data) => {
    ...
    if (variant === 'LOGIN') {
      // perform actions related to user login using NextAuth SignIn or similar authentication methods
    }
  };

  const socialAction = (action: string) => {
    setIsLoading(true);
    // NextAuth Social SignIn or similar social sign-in process
  };
